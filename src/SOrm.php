...
class SOrm
{
    ...
    public function createSelectQuery($tableName, $selectWhere = null, $selections = null, $moreSettings = null, $leftJoins = null): PromiseInterface
    {
    ...
    public function createInsertQuery($tableName, $insertArray): PromiseInterface
    {
        ...
        foreach ($keys as $key) {
            $newKeys[] = $this->escape($key);
        }

        foreach ($vals as $val) {
            $newVals[] = $this->escape($val);
        }
    ...
    public function createInsertOrDuplicateUpdateQuery($tableName, $insertArray, $updateArray): PromiseInterface
    {
        $keys = array_keys($insertArray);
        $vals = array_values($insertArray);
        ...
        foreach ($vals as $val) {
            $newVals[] = (is_null($val)) ? null : $this->escape($val);
        }
        ...
        if (count($updateArray) == 0)
            return new Promise(function (callable $resolve) {
                $resolve(['result' => false, 'error' => "Update Clause Empty"]);
            });
    ...
    public function createUpdateQuery($tableName, $updateArray, $whereArray): PromiseInterface
    {
        $newUpdateArray = [];
        $newWhereArray = [];
        ...
        if (count($updateArray) == 0 || count($whereArray) == 0)
            return new Promise(function (callable $resolve) {
                $resolve(['result' => false, 'error' => "Update OR Where Clause Empty"]);
            });
